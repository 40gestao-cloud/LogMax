...
export const ContasReceberView = ({ showToast }: any) => {
  const { data, setData, isLoading } = useFetchData<any>('/api/contasreceberview');
  ...
  const handleReceber = async (id: string) => {
    try {
      const updated = await dbUpdate('/api/contasreceberview', id, { status: 'Pago' });
      setData((prev: any[]) => prev.map(d => d.id === id ? (updated ?? { ...d, status: 'Pago' }) : d));
      showToast('Recebimento registrado!', 'success', true);
    } catch {
      showToast('Erro ao registrar recebimento.', 'error', true);
    }
  };
  ...
  const totalAberto = data.filter((c: any) => c.status === 'Aberto').reduce((sum: number, c: any) => sum + Number(c.valor ?? 0), 0);
      ...
      <div className="flex justify-between items-center shrink-0">
        ...
        <div className="flex gap-3 items-center">
          <div className="relative">
            ...
            <input type="text" placeholder="Buscar conta..." className="neu-input py-2.5 pl-10 pr-4 rounded-xl text-sm w-56"
                ...
                <AnimatePresence>
                  {enriched.map((item: any) => (
                    <motion.tr key={item.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="border-b border-white/5 hover:bg-white/5 transition-colors group">
                      ...
                      <td className="py-3 px-4 text-xs text-gray-400">{item.cliente?.nome ?? '—'}</td>
                      ...
                      <td className="py-3 px-4 text-right">
                        <div className="flex justify-end gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
                          {item.status === 'Aberto' && (
                            <button onClick={() => handleReceber(item.id)} className="neu-button py-1.5 px-3 rounded-lg text-xs font-bold text-accent hover:bg-accent/10 transition-colors flex items-center gap-1"><Check size={11} /> Receber</button>
                          )}
                          {item.status !== 'Pago' && (
                            <>
