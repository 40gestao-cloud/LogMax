...
export const ContasReceberView = ({ showToast }: any) => {
  ...
  const [extras, setExtras] = useState({ valor: '', vencimento: '', cliente_id: '', filial: FILIAL_DEFAULT as string });
  ...
  const filtered = enriched.filter((c: any) => {
    const matchSearch = [c.descricao, c.status, c.cliente?.nome].some((v: any) => v?.toLowerCase().includes(search.toLowerCase()));
  ...
  const handleExportPDF = async () => {
    try {
      ...
      const doc = new jsPDF();
      const titulo = filialFiltro ? `Contas a Receber — ${filialFiltro}` : 'Contas a Receber — Geral';
      ...
      doc.save(`logmax-contas-receber-${slug}.pdf`);
    } catch (err: any) {
  ...
  const handleExportExcel = async () => {
    const slug = filialFiltro ? filialFiltro.toLowerCase().replace(/\s/g, '-') : 'geral';
    try {
      await exportToExcel('Contas a Receber', exportCols, exportRows(), `logmax-contas-receber-${slug}`);
    } catch (err: any) {
  ...
  const openEdit = (item: any) => {
    ...
    setExtras({ valor: item.valor != null && item.valor !== '' ? formatBRL(Number(item.valor)) : '', vencimento: item.vencimento ?? '', cliente_id: item.cliente_id ?? '', filial: item.filial ?? FILIAL_DEFAULT });
    setErrors({});
    setShowForm(false);
  };
  ...
  const closeForm = () => {
    ...
    setEditItem(null);
    setForm({ descricao: '' });
    setExtras({ valor: '', vencimento: '', cliente_id: '', filial: FILIAL_DEFAULT });
    setErrors({});
  };
