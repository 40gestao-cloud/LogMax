  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col h-full gap-8">
      <div className="flex flex-wrap justify-between items-start gap-3 shrink-0">
        <div><h2 className="text-2xl sm:text-3xl font-bold text-accent tracking-tight">Inventários — {filial}</h2></div>
                        ...
                        <td className="py-3 px-4 text-right">
                          <div className="flex justify-center items-center gap-1.5">
                            ...
                            <MenuMais>
                              {fechar => (
                                <>
                                  <HistoricoOperacoes variante="menu" onAbrir={fechar} entidade="inventarios" entidadeId={item.id} titulo={`Inventário ${item.data ?? ''}`} criadoEm={item.created_at} atualizadoEm={item.updated_at} />
