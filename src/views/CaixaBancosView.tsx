...
import { Search, Edit2, Trash2, Plus, Save, Upload, X } from 'lucide-react';
...
export const CaixaBancosView = ({ showToast }: { showToast: any }) => {
  ...
  const filtered = data.filter((item: any) =>
    [item.conta, item.banco, item.agencia, item.tipo].some(v =>
  ...
  const handleImagemChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      console.error('[CaixaBancos] erro upload logo:', err);
  ...
  const handleSave = async () => {
    ...
    try {
      if (editItem) {
        ...
        // Best-effort: se a logo foi trocada/removida, apaga a antiga do bucket.
        if (imagemUrlAnterior && imagemUrlAnterior !== imagemUrl) {
          removerLogoAntiga(imagemUrlAnterior);
        }
      ...
      console.error('[CaixaBancos] erro ao salvar:', err);
  ...
  const handleDelete = async (item: any) => {
    ...
    try {
      ...
      console.error('[CaixaBancos] erro ao excluir:', err);
    ...
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col h-full gap-8">
      <div className="flex flex-wrap justify-between items-start gap-4 shrink-0">
        <div>
          ...
          <p className="text-sm text-gray-400 mt-1">Gerencie contas bancárias, saldos e logos.</p>
        </div>
              ...
              <div className="flex items-start gap-4 flex-wrap">
                ...
                <div className="flex flex-col gap-2">
                  ...
                  <div className="flex gap-2 items-center">
                    ...
                    {imagemUrl && (
                      <button onClick={handleRemoverImagem} className="neu-button py-2 px-3 rounded-xl text-xs text-gray-500 flex items-center gap-1 hover:text-red-400 transition-colors">
                        <X size={11} /> Remover
                      </button>
                    )}
                  </div>
      ...
      <div className="neu-flat rounded-3xl p-6 border border-white/5 flex flex-col mb-6">
        <div className="overflow-x-auto main-scrollbar">
          <table className="w-full text-left border-collapse">
            ...
            <tbody>
              {isLoading ? (<tr><td colSpan={8}><LoadingSpinner /></td></tr>)
                : filtered.length === 0 ? (<tr><td colSpan={8}><EmptyState message="Nenhuma conta bancária cadastrada" /></td></tr>)
                : (
                  <AnimatePresence>
