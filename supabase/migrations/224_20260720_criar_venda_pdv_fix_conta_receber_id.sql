-- =================================================================
-- LogMax — criar_venda_pdv: corrige conta_receber_id em venda parcelada
-- =================================================================
-- Bug da mig. 222: no bloco de Cartão Crédito parcelado o
-- `RETURNING id INTO v_conta_receber_id` roda dentro do FOR das
-- parcelas, então a `notas_emitidas.conta_receber_id` acaba apontando
-- só pra última parcela (12/12). Fica confuso pro operador que segue o
-- link a partir da nota.
--
-- Uma nota fiscal cobre a venda inteira, não uma parcela — quando há
-- N parcelas, o correto é deixar `conta_receber_id = NULL` na nota e
-- usar `venda_id` como ponto de amarração pro financeiro (`vendas` →
-- N contas_receber). O rastreio 1-pra-1 fica preservado só quando a
-- venda gera 1 conta (Fiado, Cartão 1x, à vista).
--
-- Idempotente. Corpo idêntico à mig. 222 exceto pelo `IF` que zera o
-- v_conta_receber_id quando a venda vira múltiplas parcelas.
-- =================================================================
...
-- Recarrega o cache de schema do PostgREST pra RPC ficar acessível
-- imediatamente pela API sem restart do projeto.
