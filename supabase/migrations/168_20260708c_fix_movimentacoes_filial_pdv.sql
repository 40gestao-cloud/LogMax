-- =================================================================
-- LogMax — Corrige filial das movimentações de estoque geradas pelo PDV
-- =================================================================
-- Bug: `movimentacoes_estoque.filial` foi adicionada em
-- 20260703j_estoque_rh_filial.sql como NOT NULL DEFAULT 'SuperMax' e
-- essa migration fez um backfill único a partir de produtos.filial —
-- mas a RPC `criar_venda_pdv` (toda venda PDV, das 3 unidades) nunca
-- foi atualizada pra passar a coluna `filial` no INSERT. Resultado:
-- toda saída de estoque por venda em MaxLook/TechMax desde então caiu
-- no DEFAULT 'SuperMax' — silencioso, nenhum erro.
--
-- Esta migration:
--   1. Reescreve criar_venda_pdv (idêntica à versão de
--      20260614_marketing_campanhas_cupons.sql) só adicionando
--      `filial` no INSERT de movimentacoes_estoque, usando v_filial
--      (já resolvida no corpo da função a partir de p_filial).
--   2. Repete o backfill de 20260703j pra corrigir linhas já erradas
--      geradas entre aquela migration e esta.
--
...
-- ─── 1. Backfill das linhas já mislabeled ──────────────────────────
...
-- ─── 2. RPC criar_venda_pdv (só o INSERT de movimentacoes_estoque muda) ──
...
BEGIN
  ...
  FOR v_item IN SELECT * FROM jsonb_array_elements(p_itens) LOOP
    ...
    -- Fix (era o bug): passa `filial` explicitamente. Antes caía sempre no
    -- DEFAULT 'SuperMax' da coluna, mesmo pra vendas de MaxLook/TechMax.
