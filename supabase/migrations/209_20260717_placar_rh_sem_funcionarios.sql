-- =================================================================
-- Fix KPI de RH no placar da competição.
--
-- Contexto:
--   Em 208, a regra "COUNT(fr.*)=0 THEN 100" premiava filiais sem
--   funcionários (ou sem registros de frequência no período) com
--   presença perfeita, distorcendo o ranking 3-2-1.
--
-- Fix:
--   Se a filial não tem funcionários ativos, valor = 0 (não compete).
--   Se tem funcionários mas nenhum registro de frequência no período,
--   valor = 0 (não pode reivindicar presença sem prova). Só calcula
--   percentual quando há registros pra medir.
--
...
BEGIN
  ...
  -- RH: taxa de presença no período. Filial sem funcionários ativos OU sem
  -- registros de frequência no período recebe 0 (não pode reivindicar
  -- presença perfeita sem prova, senão premia filial vazia).
  rh AS (
    SELECT f.filial,
      CASE
        WHEN COUNT(fu.id) = 0 THEN 0        -- filial sem funcionários ativos
        WHEN COUNT(fr.*) = 0 THEN 0         -- sem registros de frequência
