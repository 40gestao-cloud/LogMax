-- MIGR 627 — o pedido só liga produto ao catálogo pelo NOME IDÊNTICO.
--
-- Em 24/09, na LogMax-Adm, "Carregador de Tomada USB-C 20W - apple" saiu como
-- pedido de "Carregador Portátil Power Bank 20000mAh - samsung" (PC-TM-2026-0006,
-- já cancelado). A régua da migr. 596 (`vinculo_item_parece`) só exige UMA
-- palavra em comum, e "Carregador" bastou. Ela pega o absurdo (óleo x leite
-- condensado), mas não o parecido — e o parecido é o erro que acontece.
--
-- A tela deixou de escolher produto (commit eb92330): o vínculo de produto
-- nasce no cadastro com "Origem deste cadastro" (`vincular_produto_requisicao`,
-- migr. 494) ou, quando o catálogo já tem o item, pelo nome idêntico. Esta
-- migração faz o banco dizer o mesmo: quando o vínculo é feito AGORA
-- (`v_do_modal`), o nome do produto tem de ser o da requisição, comparado sem
-- acento, caixa, espaço e pontuação — a mesma normalização de `normNome` em
-- CotacoesView. PWA antiga, com o select ainda na tela, recebe a recusa.
--
-- Serviço continua na régua da 596: o cadastro de serviços não tem campo de
-- origem, e o modal de escolha é a única porta dele.
--
-- Só troca o bloco da primeira recusa do produto; o resto da função é cópia
-- do banco (md5 5a61b0febaec27261e4d3c5b1c3def00 nos 4 projetos em 24/09).

CREATE OR REPLACE FUNCTION public.nome_item_normalizado(p_texto text)
RETURNS text
LANGUAGE sql
IMMUTABLE
...
REVOKE ALL ON FUNCTION public.nome_item_normalizado(text) FROM PUBLIC, anon;
GRANT EXECUTE ON FUNCTION public.nome_item_normalizado(text) TO authenticated, service_role;
